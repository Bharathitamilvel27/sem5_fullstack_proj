...
function Admin() {
    ...
    (async () => {
      try {
        const [me, list] = await Promise.all([
          ...
          axios.get(`${API_URL}/api/tweets`, { headers: { Authorization: `Bearer ${token}` } })
        ]);
  ...
  const handleDelete = async (tweetId) => {
    ...
    try {
      await axios.delete(`${API_URL}/api/admin/tweets/${tweetId}`,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {tweets.map(t => (
              <div key={t._id} style={{ background: '#fff', border: '1px solid #e6ecf0', padding: 16, borderRadius: 12, boxShadow: '0 1px 2px rgba(0,0,0,0.03)' }}>
                ...
                {t.image && (
                  <img src={`${API_URL}${t.image}`} alt="attachment" style={{ maxWidth: '100%', borderRadius: 8, marginTop: 8 }} />
                )}
